...
//			- 참조 타입 : 객체의 참조(메모리 주소)를 저장하는 변수 타입  -(포인터와 비슷)
		
//		2. 기본 자료형
//			: 총 8가지 기본형 타입
		
//		2-1 정수 타입 (8bit == 1Byte)
//		byte : 1바이트의 정수 - 값의 범위(-128 ~ 127)
//		short : 2바이트의 정수 - 값의 범위 (-32,768 ~ 32,767)
//		int*(기본) : 4바이트의 정수 - 값의 범위 (약 -2.1억 ~ 약 2.1억)
//		long : 8바이트의 자연수 - 값의 범위 (약 -2^(63) ~ 약 2^(63))
		int age;						// 예시
		age = 27;
		System.out.println(age);
		
//		long 데이터 타입은 숫자 뒤에 알파벳 L 또는 l을 써서 long 형임을 표시
		long apartmentPrice = 100000000000L;		//예시
		System.out.println(apartmentPrice);
		
//		2-2 문자형 타입
//		char* : 하나의 문자를 저장하기 위한 변수를 선언할 때 사용
//		- 문자형의 데이터의 경우 작은 따옴표를 사용해 감싸야 한다
//		- 2바이트
		char grade = 'A';			// 예시
		char lastName = '이';
		
//		2-3 실수형(부동소수점 타입) : 소수점 자리가 있는 수 
//		float : 4바이트의 소수점 , 숫자 뒤에 알파벳 F를 써서 float형임을 표시
//		double*(기본) : 8바이트의 소수점 : 기본 실수 타입
		float height = 169.2F;			//예시
		double price = 19.3;
		
//		2-4 논리형* : 데이터 값으로 true(참), false(거짓)만 담을수 있음
//		- 1바이트 
//		- 값이 존재하는지, 결과가 참인지 거짓인지 등을 표현
//		- 소문자 값 사용을 권장
//		- is 키워드를 사용하여 해당 여부가 참인지 거짓인지 등을 표현
		boolean isMarried = false;			//예시
