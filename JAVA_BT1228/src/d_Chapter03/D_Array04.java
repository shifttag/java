...
//		matrix 배열과 array 구조가 같다
		...
		System.out.println("행의 길이 : " + threeFour.length); // 3
		System.out.println("열의 길이 : " + threeFour[0].length); // 4
		
		for(int i = 0; i < length ; i++) {		// 3번 반복
			for(int j = 0; j < innerLength; j++) {	// 4번 반복
				threeFour[i][j] = count++;
			}
		}
		...
		for(int i = 0; i < length ; i++) {		// 3번 반복
			for(int j = 0; j < innerLength; j++) {	// 4번 반복
				...
				if(threeFour[i][j] / 10 == 0) {				// if 문 생략 가능
					System.out.print(" ");
				}
			}
