...
public class A_Array01 {
	...
	public static void main(String[] args) {
		...
		// 배열을 선언하고 생성한 경우 모든 '요소'는 해당 데이터 타입의 기본 값으로 초기화
		// 문자열(''), 숫자형(0), 논리형(false)

		// 요소 : 배열 내의 각각의 데이터
		System.out.println(array); // [I@a38d7a3: 배열이 담긴 주소
...
//		System.out.println(array[3]);  - Error
		...
		System.out.print(scoreList[0] + "  "); // 배열명[인덱스] : 변수로 사용 가능
		...
		// : 배열의 길이는 고정, 한 번 생성된 배열의 크기는 변경불가
		...
		// 배열의 첫번째 요소부터 마지막 요소까지의 수 만큼 반복
		// 마지막 요소의 인덱스 = 배열의 길이 - 1
		System.out.println("for문을 사용한 배열의 순회");

		int length = fruits.length; // 코드가 길어지면 변수에 길이를 담아 작성하는것이 깔끔하게 된다.
		for (int i = 0; i < length; i++) {
			System.out.print(fruits[i] + "  ");
		}
		System.out.println();

		System.out.println("for 문을 사용한 배열의 순회2"); // 각 수를 제곱하여 출력하라
...
//			myArray 배열에 8이 존재하는지 확인
		...
		// [1]													// 1번째 반복
		// [1, 2]												// 2번째 반복
		// i가 2일 경우 continue 문을 만나 k가 증가 x					// 3번째 반복
		// i가 3일 경우 k는 2
		// : result[2] = original [3];
		// [1, 2, 4]
